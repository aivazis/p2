...
def test():
    """
    Verify that the minimum operator works as expected
    """
    # get the node class
    ...
    # construct a node that computes their minimum
    s = node.minimum(operands=nodes)

    # check the current value
    assert s.getValue() == min(range(0, 100, 10))
    ...
    # check again
    assert s.getValue() == min(range(0, 200, 20))
