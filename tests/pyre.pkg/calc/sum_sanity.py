...
def test():
    """
    Verify that the sum operator works as expected
    """
    # get the node class
    ...
    # construct a node that computes their sum
    s = node.sum(operands=nodes)

    # check the current value
    assert s.getValue() == sum(range(0, 100, 10))
    ...
    # check again
    assert s.getValue() == sum(range(0, 200, 20))
